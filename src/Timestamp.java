public class Timestamp {
    ...
    //updated Timestamp GetElement that move element to the end when needed
    public static ReturnValues GetElementImproved(ListNode l, int elementValue, int sequence, int listSize, ListNode tail) {
        ReturnValues rv = new ReturnValues();
        rv.head=null;
        rv.counter=0;
        ...
        if (ourElement!=null && ourElement.getValue()==elementValue) {
            ...
            //we should swap element only if it was requested at least once before
            //we need to search from the beginning where we should paste our element
            //Improvement. We also check that element was access last time not earlier than the list size
            //e.g. sequence id is 100, list size is 10, last time element was accessed at time 80, then we do not change the position
            if (ourElement.getLastAccess()!=-1 && sequence-ourElement.getLastAccess()<listSize/2) {
                //System.out.println("element with last time not equal -1 accessed");
                ListNode tempHead = oldhead;
                previousElement.next=ourElement.next;
                //if we should put requested element as a new head
                ...
                //otherwise we simply update the sequence
                //Or maybe we should move element to the tail?
                //System.out.println("Element with last access time -1 accessed. No specific actions needed.");
            }
            ourElement.setLastAccess(sequence);
        }
